...
void BulletManager::MakePlayerBullet(string _key, Vector3 _Position)
{
	...
	if (_key == NORMALBULLET)
	{
		...
		if (BulletLevel >= 2)
		{
			...
			(NORMALBULLET, pBullet0, Vector3(_Position.x, _Position.y - 1));
			((BulletBridge*)pBullet0)->SetTarget(1, 1);

			Bridge* pBullet1 = new NormalBullet;
			ObjectManager::GetInstance()->AddBullet
			(NORMALBULLET, pBullet1, Vector3(_Position.x, _Position.y - 1));
		...
		ObjectManager::GetInstance()->AddBullet
		(NORMALBULLET, pBullet, Vector3(_Position.x, _Position.y - 1));
	...
	else if (_key == LASERBULLET)
	{
		...
		ObjectManager::GetInstance()->AddBullet
		(LASERBULLET, pBullet, Vector3(_Position.x, _Position.y - 1));
	}
